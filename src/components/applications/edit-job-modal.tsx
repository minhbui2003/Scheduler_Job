...
interface Job {
  _id: string; position: string; status: string; notes: string; jdText: string;
...
export function EditJobModal({ applicationId, onClose, onSaved }: { applicationId: string; onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState({ companyName: '', position: '', status: 'APPLIED', notes: '', jdText: '' });
  ...
  useEffect(() => {
    ...
    fetch(`/api/applications/${applicationId}`).then(async (res) => {
      ...
      if (!active) return;
      setJob(data.data);
      setForm({ companyName: data.data.company?.name || '', position: data.data.position, status: data.data.status, notes: data.data.notes || '', jdText: data.data.jdText || '' });
